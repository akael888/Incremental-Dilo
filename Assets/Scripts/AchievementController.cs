...

{
...

    {
        ...
        if (type == AchievementType.GoldReached)
        {
            achievement = _achievementList.Find(a => a.Type == type && float.Parse(a.Value) <= float.Parse(value));
        }
        else {
        ...
        {
            if (achievement.Type == AchievementType.GoldReached)
            {
                if (float.Parse(value) >= float.Parse(achievement.Value))
                {
            ...
            else if (achievement.Type == AchievementType.ClickedReached)
            {
                if (float.Parse(value) >= float.Parse(achievement.Value))
                {
                    achievement.IsUnlocked = true;
